...
function adjustDashboardLayout() {
  ...
  // Get current width and height of dashboard-content
  const width = dashboard.offsetWidth;
  const height = dashboard.offsetHeight;
  
  // Remove both classes first
  dashboard.classList.remove('layout-condition1', 'layout-condition2');
  
  // Check conditions:
  // If width is less than height and at least 75% of height, use layout-condition1
  if (width < height && width >= 0.75 * height) {
    dashboard.classList.add('layout-condition1');
  } 
  // If width is less than 75% of height, use layout-condition2
  else if (width < 0.75 * height) {
    dashboard.classList.add('layout-condition2');
  }
  // Otherwise, no special layout (or you can add a default class if needed)
}
