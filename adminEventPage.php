...
<body>
        ...
        <div class="main-content">
            <div class="event-content">
                ...
                <div class="event-content-event-content">
                    <form action="" method="get" enctype="multipart/form-data" class="event-content-form" id="event-content-form">
                    ...
                    <div class="event-content-main-content-container">
                        <div class="event-content-table-content-container">
                            <table class="event-content-table-content">
                                <thead class="event-content-table-head">
                                    <tr class="event-content-table-row">
                                        ...
                                        <th class="event-content-data-btn-view"></th>
                                        <th class="event-content-data-btn-delete"></th>
                                    </tr>
                                </thead>
                                <tbody class="event-content-table-body">
                                    <?php 
                                        // Retrieve filter values from the GET request
                                        $status = mysqli_real_escape_string($conn, @$_GET['event-content-filter-select-status']);
                                        $col = mysqli_real_escape_string($conn, @$_GET['event-content-filter-select-col']);
                                        $searchInput = mysqli_real_escape_string($conn, @$_GET['event-content-search-input']);
                                        ...
                                        if ($status !== "") {
                                            $whereConditions[] = "event.event_status = '$status'";
                                        }
                                        if ($organizationID !== "") {
                                            $whereConditions[] = "event.event_organizer = '$organizationID'";
                                        }
                                        if (!empty($col) && !empty($searchInput)) {
                                            $whereConditions[] = "LOWER(event.$col) LIKE LOWER('%$searchInput%')";
                                        }

                                        // Combine conditions to form the WHERE clause
                                        $whereClause = !empty($whereConditions) ? "WHERE " . implode(" AND ", $whereConditions) : "";

                                        // Query to fetch event data with a single reference to the organization table
                                        $eventQuery = "SELECT event.*, organization.organization_name 
                                                    FROM event
                                                ...
                                                echo '
                                                    <tr class="event-content-table-row">
                                                        <form action="adminEventDataHandler.php" method="post">
                                                            <input type="hidden" name="event-id" value="'. $row['event_id'] .'">
                                                            <td class="event-content-data-name">'. htmlspecialchars($row['event_name']) .'</td>
                                                            <td class="event-content-data-place">'. htmlspecialchars($row['event_place']) .'</td>
                                                            <td class="event-content-data-status">'. htmlspecialchars($statusText) .'</td>
                                                            <td class="event-content-data-date">'. $eventDate .'</td>
                                                            <td class="event-content-data-time">'. $eventTime .'</td>
                                                            <td class="event-content-data-organizer">'. htmlspecialchars($row['organization_name']) .'</td>
                                                            <td class="event-content-data-btn-view">
                                                                <button type="submit" name="vSubmit" class="user-view">VIEW</button>
                                                            </td>
                                                            <td class="event-content-data-btn-delete">
    ...
    <script>
        ...
        // const confirmSubmit = confirm('Are you sure you want to add this event?');
        
        // const form = document.querySelector('.add-new-event-form-content');
    ...
    //     // Show loading indicator or perform other actions (optional)
    //     const loadingIndicator = document.createElement('div');
    //     loadingIndicator.innerText = 'Submitting form...';
    //     document.body.appendChild(loadingIndicator);

    //     // Perform the fetch request
    //     fetch('adminEventDataHandler.php', {
    //         method: 'POST',
    ...
    //             // Optionally redirect or update the UI based on response
    //             window.location.href = 'adminEventPage.php'; // Redirect after success
    //         } else {
    ...
    //     })
    //     .finally(() => {
    //         document.body.removeChild(loadingIndicator); // Remove the loading indicator
    //     });
    // });
...
function capitalize(string) {
    return string.charAt(0).toUpperCase() + string.slice(1);
        ...
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Initialize the participant value
            updateParticipants();
        ...
        // Store selected programs and years
